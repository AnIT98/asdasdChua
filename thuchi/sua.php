...
if(isset($_GET['id'])){
$id=$_GET['id'];
    $sql = "SELECT * FROM `thuchi` WHERE id = $id ";
    // echo $sql;
    // exit();
...
if(isset($_POST['btnluu'])){
    ...
    $dg=$_POST['dg'];
    $tong=$_POST['tong'];
    $thuorchi=$_POST['thuorchi'];


    $sqlluu="UPDATE `thuchi` SET `ngay`='$ngay',`mota`='$mota',`thuorchi`='$thuorchi',`soluong`='$sl',`dongia`='$dg',`tongtien`='$tong' WHERE `id` =  $idluu";
   ...
   <?php include 'menu.php'; ?>
    ...
    <div class="row">
        <div id="f-thuchi" class="col-12 d-flex justify-content-center mb-5" style="">
            <form style="min-width:800px; position: absolute;top: 9%; left: 18%; z-index:999" action="sua.php"
                method="POST">
                <h3>Sửa Lại</h3>
                <div class="form-group d-none">
                    <label for="exampleFormControlTextarea1">STT</label><br>
                    <input type="text" id="text" name="id_sua" value="<?php echo $row["id"]?>">
                </div>
                <div class="form-group">
                    <label for="exampleFormControlTextarea1">Ngày</label><br>
                    <input type="date" id="date" name="ngay" value="<?php echo $row["ngay"]?>">
                </div>
                <div class="form-group">
                ...
                <div class="form-group mb-5">
                    ...
                    <select class="form-control" id="exampleFormControlSelect1" name="thuorchi">
                        <?php
    if ($row["thuorchi"] == 1) {
        echo('<option selected="selected" value='.'1'.'>'.'Thu'.'</option>');
    } else {
        echo('<option value='.'1'.'>'.'Thu'.'</option>');
    }
    if ($row["thuorchi"] == -1) {
        echo('<option selected="selected" value='.'-1'.'>'.'Chi'.'</option>');
    } else {
        echo('<option value='.'-1'.'>'.'Chi'.'</option>');
    }
?>
                    </select>
                </div>
